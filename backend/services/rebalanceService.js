...
const scoreStock = (stock) => {
  ...
  // 4. Future Growth (0-20 pts) — analyst target upside score 0-10 → ×2
  const futureGrowthScore = Math.min((stock.futureGrowth ?? 5) * 2, 20);
  score += futureGrowthScore;

  // 5. Social/Sentiment (0-15 pts) — momentum score 0-10 → ×1.5
 ...
 * liveResults is an array (same order as tickers) of enriched objects or null.
 * universeDefs is the array of { ticker, companyName } from STOCK_UNIVERSE.
 */
...
const rebalanceBasket = async (basketId, manualTrigger = false) => {
  try {
    ...
    // Fetch live data with per-ticker fallback
    let enrichedStocks;
    try {
    ...
    basket.minimumInvestment   = Math.ceil(newStocks.reduce((sum, s) => sum + ((s.currentPrice || 0) * (s.quantity || 1)), 0));
